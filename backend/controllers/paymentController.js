...
const hasRazorpay = process.env.RAZORPAY_KEY_ID && process.env.RAZORPAY_KEY_SECRET && !process.env.RAZORPAY_KEY_ID.includes('your-');
...
exports.createOrder = async (req, res) => {
  try {
    ...
    if (user.hasPaid) return res.json({ alreadyPaid: true });

    let amountInPaise = 50000;
    if (courseId) {
      const course = await Course.findById(courseId);
      if (course && course.price) amountInPaise = course.price * 100;
    }
...
exports.verifyPayment = async (req, res) => {
  try {
    ...
    if (courseId) await ensureEnrollment(user._id, courseId);

    res.json({ success: true, mock: !!mock });
  } catch (err) {
